...
**强制要求：所有代码提交必须包含单元测试，且测试必须 100% 通过。**

### 测试原则

1. **测试先行** - 新功能开发前先写测试用例，修复 bug 前先写复现测试
2. **100% 通过** - 所有测试必须通过才能提交，不允许跳过失败的测试
3. **核心覆盖** - 业务逻辑、工具函数、API 端点必须有测试覆盖
4. **快速执行** - 单元测试应在秒级完成，保持开发效率
5. **持续维护** - 代码变更时同步更新测试，删除过时测试
...
uv run pytest tests/test_tools/test_screener.py  # 运行特定文件
uv run pytest --cov=src/moatlab                  # 查看覆盖率
```
